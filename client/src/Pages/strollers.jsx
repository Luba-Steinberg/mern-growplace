...
function StrollersPage() {
  const dispatch = useDispatch()
  const [strollersProducts, setStrollersProducts] = React.useState([])
  //add for each category page
  ...
  const getDataForCategory = async () => {
    ...
    const options = {
      ...
      body: JSON.stringify({ category: 'strollers', user: user._id }),
    }
    try {
      ...
      if (response.ok) {
        setStrollersProducts(myData.products)
        console.log('the products set by setStrollersProducts are', strollersProducts)
      } else {
        dispatch(SetLoader(false))
