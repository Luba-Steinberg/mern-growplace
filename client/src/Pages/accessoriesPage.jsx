...
function AccessoriesPage() {
  ...
  const [accessoriesProducts, setAccessoriesProducts] = React.useState([])
  ...
  const getDataForCategory = async () => {
    ...
    const options = {
      ...
      body: JSON.stringify({ category: 'accessories', user: user._id }),
    }
    try {
      ...
      if (response.ok) {
        setAccessoriesProducts(myData.products)
        console.log('the products set by setAccessoriesProducts are', accessoriesProducts)
      } else {
        dispatch(SetLoader(false))
  ...
  return (
    <div className="row" style={{ backgroundColor: '#eee' }}>
      {/* <AsideAccessories></AsideAccessories> */}
      <section
