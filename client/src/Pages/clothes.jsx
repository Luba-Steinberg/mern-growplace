...
function ClothesPage() {
  ...
  const getDataForCategory = async () => {
    ...
    const options = {
      ...
      body: JSON.stringify({ category: 'clothes', user: user._id }),
    }
    try {
      ...
      if (response.ok) {
        setClothesProducts(myData.products)
        console.log('the products set by setClothesProducts are', clothesProducts)
      } else {
        dispatch(SetLoader(false))
